...
use super::{
    graph::CodegenGraph,
    naming::{CargoFeature, CodegenIdentUsage, CodegenTypeName},
...
impl<'a> CodegenRef<'a> {
    pub fn new(graph: &'a CodegenGraph<'a>, ty: &'a TypeView<'a>) -> Self {
        Self { graph, ty }
    }
}
...
impl ToTokens for CodegenRef<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.append_all(match self.ty {
            ...
            TypeView::Inline(ty) => {
                ...
                let root: syn::Path = match path.root {
                    ...
                    InlineTypePathRoot::Type(name) => {
                        let mod_name = self.graph.schema_ident(name).as_module();
            ...
            TypeView::Schema(ty) => {
                let ty_name = self.graph.schema_ident(ty.name()).as_type();
