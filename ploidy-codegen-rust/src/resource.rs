...
use super::{
    cfg::CfgFeature,
    graph::{CodegenGraph, CodegenOperationView},
...
pub struct CodegenResource<'a> {
    ...
    module_ident: CodegenResourceIdent<'a>,
    ops: &'a [CodegenOperationView<'a, 'a>],
}
...
impl IntoCode for CodegenResource<'_> {
    ...
    fn into_code(self) -> Self::Code {
        (
            format!(
                "src/client/{}.rs",
                CodegenIdentUsage::Module(&self.module_ident).display()
            ),
