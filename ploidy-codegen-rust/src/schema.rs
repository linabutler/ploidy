use ploidy_core::{
    codegen::IntoCode,
    ir::{ContainerView, SchemaTypeInfo, SchemaTypeView, TypeInfo, View},
};
...
impl ToTokens for CodegenSchemaType<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let info = SchemaTypeInfo {
            name: s.name(),
            resource: s.resource(),
        };
        let info = TypeInfo::Schema(info);
        let ty = match s {
            SchemaTypeView::Struct(_, view) => {
                CodegenStruct::new(self.graph, info, view).into_token_stream()
            }
            SchemaTypeView::Enum(_, view) => {
                CodegenEnum::new(self.graph, info, view).into_token_stream()
            }
            SchemaTypeView::Tagged(_, view) => {
                CodegenTagged::new(self.graph, info, view).into_token_stream()
            }
            SchemaTypeView::Untagged(_, view) => {
                CodegenUntagged::new(self.graph, info, view).into_token_stream()
            }
            SchemaTypeView::Container(_, ContainerView::Array(inner)) => {
                let doc_attrs = inner.description().map(doc_attrs);
                let type_name = CodegenIdentUsage::Type(&self.graph.ident(info));
...
impl IntoCode for CodegenSchemaType<'_> {
    ...
    fn into_code(self) -> Self::Code {
        let ident = self.graph.ident(self.ty.info());
