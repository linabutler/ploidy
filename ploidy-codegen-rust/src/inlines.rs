...
use ploidy_core::ir::{Identifiable, InlineTypeView};
...
use super::{
    cfg::CfgFeature, enum_::CodegenEnum, graph::CodegenGraph, naming::format_inline_type_path,
...
impl<'a> CodegenInlines<'a> {
    ...
    pub fn new(
        ...
        inlines.sort_by_cached_key(|view| format_inline_type_path(graph, view.path()));
        Self {
            graph,
            inlines,
            cfg: false,
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let mut items = self.inlines.iter().filter_map(|view| {
            let ty = match view {
                ...
                InlineTypeView::Enum(_, view) => CodegenEnum::new(graph, view).into_token_stream(),
            ...
            Some(if self.cfg {
                ...
                let mod_name = CodegenIdentUsage::Module(&graph.ident(view.id()));
                quote! {
                    #cfg
                    mod #mod_name {
                        #ty
