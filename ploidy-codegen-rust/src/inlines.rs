...
use ploidy_core::ir::{InlineTypeView, TypeInfo};
use proc_macro2::TokenStream;
use quote::{ToTokens, TokenStreamExt, quote};

use crate::CodegenIdentUsage;

use super::{
    cfg::CfgFeature, enum_::CodegenEnum, graph::CodegenGraph, struct_::CodegenStruct,
...
impl<'a> CodegenInlines<'a> {
    ...
    pub fn new(
        ...
        inlines.sort_by_cached_key(|view| view.trace());
        Self {
            graph,
            inlines,
            cfg: false,
...
impl ToTokens for CodegenInlines<'_> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let mut items = self.inlines.iter().filter_map(|view| {
            let info = TypeInfo::Inline(view.id());
            let ty = match view {
                InlineTypeView::Struct(_, _, view) => {
                    CodegenStruct::new(graph, info, view).into_token_stream()
                }
                InlineTypeView::Enum(_, _, view) => {
                    CodegenEnum::new(graph, info, view).into_token_stream()
                }
                InlineTypeView::Tagged(_, _, view) => {
                    CodegenTagged::new(graph, info, view).into_token_stream()
                }
                InlineTypeView::Untagged(_, _, view) => {
                    CodegenUntagged::new(graph, info, view).into_token_stream()
                }
            ...
            Some(if self.cfg {
                ...
                let mod_name = CodegenIdentUsage::Module(&graph.ident(info));
                quote! {
                    #cfg
                    mod #mod_name {
                        #ty
