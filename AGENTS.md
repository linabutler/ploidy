...
- **View types** (e.g., `StructView`, `TaggedView`, `EnumView`, `OperationView`): Graph-aware wrappers providing `inlines()`, `traverse()`, `used_by()`, `needs_box()`, and metadata access.
...
"Duplication is far cheaper than the wrong abstraction." Let repetition exist until the *actual* shared structure reveals itself, then extract. The test: the abstraction must simplify *every* call site. If any caller needs a special flag or `if` branch, keep it inline.
...
- **No new helper functions**. Inline all fixtures directly. Use existing helpers, don't add new ones without asking. Abstraction discipline applies doubly in tests.
