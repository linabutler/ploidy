...
fn test_parents_returns_immediate_parents() {
    ...
    let mut field_names = user_struct
        ...
        .collect_vec();
    field_names.sort();
    assert_matches!(&*field_names, ["email", "id", "name"]);
}
...
fn test_all_of_inheritance_with_fields() {
    ...
    let mut all_field_names = child_struct
        .fields()
        .map(|f| match f.name() {
            IrStructFieldName::Name(n) => n,
    ...
    assert_matches!(&*all_field_names, ["child_field", "parent_field"]);
...
fn test_multiple_parents() {
    ...
    let mut parent_names = combined_struct
        .parents()
        .filter_map(|p| p.into_schema().ok())
        .map(|s| s.name())
        .collect_vec();
...
fn test_circular_all_of_terminates() {
    ...
    let mut field_names = a_struct
        ...
        .collect_vec();
    field_names.sort();
    assert_matches!(&*field_names, ["a_field", "b_field", "kind"]);
